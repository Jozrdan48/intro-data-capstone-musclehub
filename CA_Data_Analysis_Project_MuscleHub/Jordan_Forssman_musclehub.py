...
_, pval1, _, _ = chi2_contingency(x)
print pval1
...
is_member = just_apps.groupby(['ab_test_group', 'is_member']).first_name.count().reset_index()
is_member

member_pivot = is_member.pivot(columns = 'is_member', index = 'ab_test_group', values='first_name' ).reset_index()
member_pivot
...
_, pval2, _, _ = chi2_contingency(x)
print pval2
...
_, pval3, _, _ = chi2_contingency(x)
...
plt.show()
plt.savefig('percent_with_application.png')
...
ax.set_title('Percent of Applicants who Purchase')
